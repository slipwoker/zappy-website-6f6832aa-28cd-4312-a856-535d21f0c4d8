document.addEventListener('DOMContentLoaded', function() {
  ...
  if (navbar) {
    ...
    window.addEventListener('scroll', function() {
      ...
      scrollTimeout = requestAnimationFrame(updateNavbarState);
    }, { passive: true });
    ...
    function validateField(input) {
      ...
      if (input.type === 'email' && value !== '') {
        if (!validateEmail(value)) {
          showError(input, isRTL ? 'يرجى إدخال بريد إلكتروني صحيح' : 'Please enter a valid email address');
          return false;
        }
      }
      ...
      if (input.type === 'tel' && value !== '') {
        ...
        if (!phoneRegex.test(value)) {
          showError(input, isRTL ? 'يرجى إدخال رقم هاتف صحيح' : 'Please enter a valid phone number');
          return false;
        }
      }
      ...
      if (input.hasAttribute('minlength') && value !== '') {
        ...
        if (value.length < minLength) {
          showError(input, isRTL ? `الحد الأدنى ${minLength} حرف` : `Minimum ${minLength} characters required`);
          return false;
        }
      }
    ...
    contactForm.addEventListener('submit', function(e) {
      ...
      if (isValid) {
        ...
        if (successMessage) {
          ...
          successMessage.textContent = isRTL ? 'تم إرسال النموذج بنجاح!' : 'Form submitted successfully!';
        } else {
          const messageEl = document.createElement('div');
          messageEl.className = 'form-success-message';
          messageEl.textContent = isRTL ? 'تم إرسال النموذج بنجاح!' : 'Form submitted successfully!';
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '6f6832aa-28cd-4312-a856-535d21f0c4d8',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '6f6832aa-28cd-4312-a856-535d21f0c4d8';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '6f6832aa-28cd-4312-a856-535d21f0c4d8';
